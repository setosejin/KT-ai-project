...
static void __query_request_body_handler(IMPacketBodyPtr pbodyResp, IMPacketBodyPtr pbodyRqst)
{
	...
	IMCbIF333Hndl if333_handler= (IMCbIF333Hndl)im_base_get_cb_if333_hndler();

	rc = im_if333_body_get_extrSysId(pbodyRqst, &extrSysId);
	rc = im_if333_body_get_cmdDataInfo_with_index(pbodyRqst, 0, &cmdType);

	if ( if333_handler != NULL )	
	{
		for (idx=0; idx<IOTMAKERS_MAX_CONTROL_DATA; idx++)	{
			rc = im_if333_body_get_inclDevId_with_index(pbodyRqst, idx, &inclDevId);
			if ( rc < 0 )	{
				break;
			}
			
			im_if333_devBasVO_append_init(pbodyResp, extrSysId, inclDevId);

			if333_handler(pbodyRqst, inclDevId, cmdType);
		}

	} else {
		INFO_LOG("No if333_handler set");
	}
...
static int im_action_query_send_rsp(IMPacketPtr pktRqst)
{
	...
	im_packet_set_head_methodType_as_INITA_DEV_INFO(ppkt);

	im_pktHead_copy_trmTransacId(&(pkt.head), &(pktRqst->head));

	im_if333_init_resp_body(pbody, pbodyRqst);	
...
int im_action_if333_req_handler(IMPacketPtr reqPkt)
{
	int rc = 0;
